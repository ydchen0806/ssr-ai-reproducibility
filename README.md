...
the [displayed seed list](docs/BEST_SEEDS.md),
...
Displayed visual inference checkpoints (VOC n=5 primary seeds, the Figure 6c
example, and the CUB rank-32 response pair) are public on
[Hugging Face](https://huggingface.co/cyd0806/ssr-ai-checkpoints).
Original per-seed logs for the main-text Figure 4–6 experiments are in
[`main_text_experiment_records/`](https://huggingface.co/cyd0806/ssr-ai-checkpoints/tree/main/main_text_experiment_records).
The [download index](docs/checkpoint_downloads.md) links each paired seed and arm.
Historical KE runs saved evaluation records without exporting adapters; see the
[persistence audit](docs/ke_checkpoint_audit.md).
...
development selector, and compact Figure 5 source tables. Displayed panels
report n=3–5 paired seeds; original per-seed logs are on Hugging Face.
...
result record, and `python3 scripts/verify_paper_20260810.py`
...
`artifacts/paper_20260810` contains the archived CounterFact factorial campaign
