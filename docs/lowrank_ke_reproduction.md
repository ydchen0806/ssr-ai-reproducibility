...
Current main-text Figure 5 reports **n=5 displayed WikiRecent orders per rank**, identical to the main-text panels.
The commands below remain the historical launcher. Do not treat leftover unselected orders as the displayed manuscript statistics.
...
a 250-edit rank cohort and a separate 250-edit mechanism cohort. Current main-text Figure 5 reports five paired orders per rank; historical campaign files launched ten orders. The panel asks whether the acquisition
...
tuple before their paired streams finish.
...
`(LoRA + the same anchor + SSR)`, with the same development/freeze/
