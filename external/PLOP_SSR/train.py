...
from teacher_ssr import classifier_channel_ssr_penalty
...
class Trainer:
    ...
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, step=0):
        ...
        self.device = device
        self.step = step
        self.ssr_lambda = opts.ssr_lambda
        self.ssr_target_scope = opts.ssr_target_scope
        self.ssr_gradient_gate = opts.ssr_gradient_gate
        self.ssr_warmup_epochs = opts.ssr_warmup_epochs
        self.ssr_normalization = opts.ssr_normalization
        self.ssr_kernel = {
            "a_exc": opts.ssr_a_exc,
            "a_inh": opts.ssr_a_inh,
            "sigma_exc": opts.ssr_sigma_exc,
            "sigma_inh": opts.ssr_sigma_inh,
            "distance": opts.ssr_distance,
            "target_scope": opts.ssr_target_scope,
            "repulsion_margin": opts.ssr_repulsion_margin,
        }
        self.ssr_start_step = opts.ssr_start_step
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            primary_loss = loss + lkd + lde + l_icarl + pod_loss + loss_entmin
            l_ssr = loss.new_zeros(())
            ssr_gate = loss.new_ones(())
            ssr_alignment = loss.new_zeros(())
            ssr_ramp = loss.new_ones(())
            ssr_scale = loss.new_ones(())
            if self.ssr_lambda > 0.0 and self.step >= self.ssr_start_step:
                # Keep the regularizer in FP32 even when the main objective
                # is trained under AMP.
                with torch.cuda.amp.autocast(enabled=False):
                    l_ssr = classifier_channel_ssr_penalty(
                        model.module.cls, **self.ssr_kernel
                    ).to(dtype=loss.dtype)
                if not torch.isfinite(l_ssr):
                    raise FloatingPointError("SSR penalty became non-finite")
                if self.ssr_warmup_epochs > 0:
                    ssr_ramp = l_ssr.new_tensor(
                        min(1.0, float(cur_epoch + 1) / self.ssr_warmup_epochs)
                    )
                if self.ssr_gradient_gate == "nonconflicting_v1":
                    heads = model.module.cls[:-1] if self.ssr_target_scope == "historical_only" else model.module.cls[-1:]
                    ssr_parameters = []
                    for head in heads:
                        if hasattr(head, "ssr_parameters"):
                            ssr_parameters.extend(
                                parameter for parameter in head.ssr_parameters()
                                if parameter.requires_grad
                            )
                        elif head.weight.requires_grad:
                            ssr_parameters.append(head.weight)
                    primary_grads = torch.autograd.grad(
                        primary_loss, ssr_parameters, retain_graph=True,
                        allow_unused=True,
                    )
                    auxiliary_grads = torch.autograd.grad(
                        l_ssr, ssr_parameters, retain_graph=True,
                        allow_unused=True,
                    )
                    paired_grads = [
                        (left, right) for left, right in zip(primary_grads, auxiliary_grads)
                        if left is not None and right is not None
                    ]
                    if paired_grads:
                        dot = sum((left * right).sum() for left, right in paired_grads)
                        primary_norm = torch.sqrt(sum(left.square().sum() for left, _ in paired_grads)).clamp_min(1e-12)
                        auxiliary_norm = torch.sqrt(sum(right.square().sum() for _, right in paired_grads)).clamp_min(1e-12)
                        ssr_alignment = (dot / (primary_norm * auxiliary_norm)).detach()
                        ssr_gate = (ssr_alignment >= 0.0).to(dtype=l_ssr.dtype)
                    else:
                        ssr_gate = l_ssr.new_zeros(())
                if self.ssr_normalization == "relative_primary_v1":
                    ssr_scale = torch.where(
                        l_ssr.detach().abs() > 1e-12,
                        primary_loss.detach().abs() / l_ssr.detach().abs().clamp_min(1e-12),
                        l_ssr.new_zeros(()),
                    ).clamp(max=1e6)
            ssr_contribution = self.ssr_lambda * ssr_ramp * ssr_gate * ssr_scale * l_ssr
            loss_tot = primary_loss + ssr_contribution
            if not torch.isfinite(loss_tot):
            ...
            reg_loss += lkd.item() + lde.item() + l_icarl.item() + ssr_contribution.item()
            interval_loss += loss.item() + lkd.item() + lde.item() + l_icarl.item() + pod_loss.item(
            ) + loss_entmin.item() + ssr_contribution.item()
            ...
            if (cur_step + 1) % print_int == 0:
                ...
                logger.info(
                    ...
                    f"POD {pod_loss} SSR {l_ssr} SSRScaled {ssr_contribution} "
                    f"SSRGate {ssr_gate} SSRAlignment {ssr_alignment} SSRRamp {ssr_ramp} "
                    f"SSRScale {ssr_scale} EntMin {loss_entmin}"
                )
                # visualization
    ...
    def validate(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False):
        ...
        l_icarl = torch.tensor(0.)
        l_reg = torch.tensor(0.)
        old_boundary_margin_sum = torch.zeros((), device=device)
        old_boundary_margin_count = torch.zeros((), device=device)
        old_boundary_pixel_count = torch.zeros((), device=device, dtype=torch.long)
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                if self.step > 0 and self.old_classes > 1 and outputs.shape[1] > self.old_classes:
                    old_boundary = metrics.boundary_mask(labels) & (labels > 0) & (labels < self.old_classes)
                    if old_boundary.any():
                        margin = (
                            outputs[:, :self.old_classes].amax(dim=1)
                            - outputs[:, self.old_classes:].amax(dim=1)
                        )
                        old_boundary_values = margin.masked_select(old_boundary)
                        if not torch.isfinite(old_boundary_values).all():
                            raise FloatingPointError("Old-boundary logit margin became non-finite")
                        old_boundary_margin_sum += old_boundary_values.sum()
                        old_boundary_margin_count += old_boundary_values.numel()
                        old_boundary_pixel_count += old_boundary.sum()
            ...
            torch.distributed.reduce(old_boundary_margin_sum, dst=0)
            torch.distributed.reduce(old_boundary_margin_count, dst=0)
            torch.distributed.reduce(old_boundary_pixel_count, dst=0)
            if distributed.get_rank() == 0:
                if old_boundary_margin_count.item() > 0:
                    score["Old Boundary Logit Margin"] = (
                        old_boundary_margin_sum / old_boundary_margin_count
                    ).item()
                else:
                    score["Old Boundary Logit Margin"] = float("nan")
                score["Old Boundary Pixel Count"] = int(old_boundary_pixel_count.item())
                old_ids = range(1, self.old_classes)
                old_biou = [score["Boundary Class IoU"].get(c) for c in old_ids]
                old_biou = [
                    float(value) for value in old_biou
                    if isinstance(value, Real) and math.isfinite(float(value))
                ]
                score["Old Boundary IoU"] = sum(old_biou) / len(old_biou) if old_biou else float("nan")
