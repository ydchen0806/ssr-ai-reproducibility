...
def main(opts):
    ...
    if rank == 0 and opts.teacher_metrics_json:
        directory = os.path.dirname(os.path.abspath(opts.teacher_metrics_json))
        ...
        def serializable(value):
            ...
            return None
        payload = {
            "schema_version": "plop_ssr_lowrank_teacher_metrics_v2",
            "dataset": opts.dataset,
            "task": opts.task,
            "method": opts.method,
            "random_seed": opts.random_seed,
            "ssr": {
                "lambda": opts.ssr_lambda,
                "normalization": opts.ssr_normalization,
                ...
                "start_step": opts.ssr_start_step,
                "target_scope": opts.ssr_target_scope,
                "gradient_gate": opts.ssr_gradient_gate,
                "warmup_epochs": opts.ssr_warmup_epochs,
                "repulsion_margin": opts.ssr_repulsion_margin,
                "target": (
                    "effective_historical_low_rank_decoder_geometry"
                    if opts.low_rank_classifier_rank > 0 else
                    "incremental_decoder_classifier_channels"
                ),
            },
            "low_rank_classifier": {
                "rank": opts.low_rank_classifier_rank,
                "alpha": opts.low_rank_classifier_alpha,
                "scope": "historical_decoder_classifier_residual",
            },
            "steps": [serializable(score) for score in all_val_scores],
        }
        with open(opts.teacher_metrics_json, "w") as handle:
...
def run_step(opts, world_size, rank, device):
    ...
    if True: #TRAIN:
        ...
        del checkpoint
        trainer = Trainer(
            model, None, device=device, opts=opts,
            classes=tasks.get_per_task_classes(opts.dataset, opts.task, opts.step),
            step=opts.step,
        )
