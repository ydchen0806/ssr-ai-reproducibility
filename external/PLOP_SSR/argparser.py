...
def modify_command_options(opts):
    if opts.ssr_lambda < 0:
        raise ValueError("--ssr-lambda must be non-negative")
    if opts.ssr_sigma_exc <= 0 or opts.ssr_sigma_inh <= 0:
        raise ValueError("SSR kernel widths must be positive")
    if opts.ssr_start_step < 0:
        raise ValueError("--ssr-start-step must be non-negative")
    if opts.low_rank_classifier_rank < 0:
        raise ValueError("--low-rank-classifier-rank must be non-negative")
    if opts.low_rank_classifier_rank > 0 and opts.low_rank_classifier_alpha <= 0:
        raise ValueError("--low-rank-classifier-alpha must be positive when low rank is enabled")
    if opts.dataset == 'voc':
        opts.num_classes = 21
...
def get_argparser():
    ...
    # Performance Options
    # PyTorch 2.x launchers pass --local-rank; older launchers use --local_rank.
    # Keep both spellings mapped to the original field used by run.py.
    ...
    # Teacher-revision SSR: operates on the incremental decoder classifier
    # channels.  A zero coefficient is an exact no-op, so PLOP is preserved.
    parser.add_argument("--ssr-lambda", default=0.0, type=float)
    parser.add_argument("--ssr-a-exc", default=1.0, type=float)
    parser.add_argument("--ssr-a-inh", default=0.8, type=float)
    parser.add_argument("--ssr-sigma-exc", default=0.2, type=float)
    parser.add_argument("--ssr-sigma-inh", default=0.5, type=float)
    parser.add_argument(
        "--ssr-distance", default="projective", choices=["projective", "cosine"]
    )
    parser.add_argument(
        "--ssr-start-step", default=1, type=int,
        help="First incremental step at which SSR is active; step 0 has no prior memory to stabilize.",
    )
    parser.add_argument(
        "--ssr-target-scope", default="current_to_old",
        choices=["current_to_old", "historical_only"],
    )
    parser.add_argument(
        "--ssr-gradient-gate", default="none",
        choices=["none", "nonconflicting_v1"],
    )
    parser.add_argument(
        "--ssr-warmup-epochs", default=0, type=int,
        help="Linearly ramp SSR from zero over this many epochs per increment.",
    )
    parser.add_argument("--ssr-repulsion-margin", default=0.15, type=float)
    parser.add_argument(
        "--ssr-normalization", default="none",
        choices=["none", "relative_primary_v1"],
        help="Optionally scale SSR relative to the detached primary objective.",
    )
    parser.add_argument(
        "--low-rank-classifier-rank", default=0, type=int,
        help="Rank of the trainable residual on historical decoder classifiers; zero disables it.",
    )
    parser.add_argument(
        "--low-rank-classifier-alpha", default=1.0, type=float,
        help="Residual scale numerator; the effective multiplier is alpha/rank.",
    )
    parser.add_argument(
        "--teacher-metrics-json", default=None, type=str,
        help="Optional machine-readable per-stream metric export for the SSR campaign."
    )
