...
class LowRankResidualConv2d(nn.Conv2d):
    """Frozen 1x1 classifier plus a trainable low-rank residual."""

    def __init__(self, in_channels, out_channels, rank, alpha, bias=True):
        super().__init__(in_channels, out_channels, kernel_size=1, bias=bias)
        requested_rank = int(rank)
        requested_alpha = float(alpha)
        if requested_rank <= 0:
            raise ValueError("low-rank classifier rank must be positive")
        if not math.isfinite(requested_alpha):
            raise ValueError("low-rank classifier alpha must be finite")

        effective_rank = min(requested_rank, in_channels, out_channels)
        effective_alpha = requested_alpha * effective_rank / requested_rank
        self.low_rank_requested_rank = requested_rank
        self.low_rank_requested_alpha = requested_alpha
        self.low_rank_rank = effective_rank
        self.low_rank_alpha = effective_alpha
        self.lora_A = nn.Parameter(torch.empty(effective_rank, in_channels))
        self.lora_B = nn.Parameter(torch.zeros(out_channels, effective_rank))
    ...
    def effective_weight(self):
        scale = self.low_rank_alpha / self.low_rank_rank
        residual = (self.lora_B @ self.lora_A).view_as(self.weight)
        return self.weight + scale * residual

    def forward(self, inputs):
        return F.conv2d(inputs, self.effective_weight(), self.bias)

    def ssr_parameters(self):
        return (self.lora_A, self.lora_B)


def classifier_weight(module):
    return module.effective_weight() if hasattr(module, "effective_weight") else module.weight
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(
        ...
        low_rank = int(getattr(opts, "low_rank_classifier_rank", 0))
        low_rank_alpha = float(getattr(opts, "low_rank_classifier_alpha", 1.0))
        heads = []
        for index, class_count in enumerate(classes):
            if low_rank > 0 and len(classes) > 1 and index < len(classes) - 1:
                heads.append(LowRankResidualConv2d(
                    head_channels, class_count, low_rank, low_rank_alpha, bias=use_bias
                ))
            else:
                heads.append(nn.Conv2d(head_channels, class_count, 1, bias=use_bias))
        self.cls = nn.ModuleList(heads)
    ...
    def _compute_weights_norm(self, convs, only="all"):
        ...
        for i, conv in enumerate(convs):
            w = classifier_weight(conv).detach()[..., 0, 0]
        ...
        if only_bg:
            ...
            for i, mod in enumerate(self.cls):
                if i == 0 and self.multi_modal_background:
                    ...
                    w = F.normalize(classifier_weight(mod), dim=1, p=2)
    ...
    def init_new_classifier(self, device):
        ...
        if self.multi_modal_background:
            imprinting_w = classifier_weight(self.cls[0]).sum(dim=0)
            bkg_bias = self.cls[0].bias.sum(dim=0)
        else:
            imprinting_w = classifier_weight(self.cls[0])[0]
