...
Runs KE-specific methods (ROME, MEMIT, AlphaEdit, SPHERE, WISE) with the
same data loader and provenance manifest as the SSR fine-tuning study on
KnowEdit (ZsRE, WikiCounterfact, WikiRecent). Generic EasyEdit outputs are
editor-native diagnostics; long-stream manuscript comparisons require the
separate matched retention and frozen-control evaluator.
...
"""
import sys, os, json, argparse, logging, time, random, hashlib, subprocess, importlib
from contextlib import contextmanager
from pathlib import Path

PROJECT_DIR = Path(os.environ.get("SSR_REPO_DIR", Path(__file__).resolve().parents[2])).resolve()
DATASET_ROOT = Path(os.environ.get("SSR_DATASET_ROOT", PROJECT_DIR / "dataset")).resolve()
...
DATASET_MAP = {
    "zsre": str(DATASET_ROOT / "knowedit/benchmark/ZsRE/ZsRE-test-all.json"),
    "cf": str(DATASET_ROOT / "knowedit/benchmark/wiki_counterfact/test_cf.json"),
    "recent": str(DATASET_ROOT / "knowedit/benchmark/wiki_recent/recent_test.json"),
    "wikibio": str(DATASET_ROOT / "knowedit/benchmark/WikiBio/wikibio-test-all.json"),
}
...
# Keep every EasyEdit baseline on the same runner so that source revision,
# model reference, data slice, and hparameter checksum are recorded alike.
EASYEDIT_METHODS = {
    "ROME": ("ROMEHyperParams", "ROME"),
    "MEMIT": ("MEMITHyperParams", "MEMIT"),
    "ALPHAEDIT": ("AlphaEditHyperParams", "AlphaEdit"),
    "SPHERE": ("SPHEREHyperParams", "SPHERE"),
    "WISE": ("WISEHyperParams", "WISE"),
}


def _biocs_editor_module():
    """Load the explicitly selected, versioned custom editor module."""
    module_name = os.environ.get(
        "BIOCS_EDITOR_MODULE", "llm_ke.biocs_editor_history_v1"
    ).strip()
    if not module_name.startswith("llm_ke."):
        raise ValueError("BIOCS_EDITOR_MODULE must be a module within llm_ke")
    return importlib.import_module(module_name)


def easyedit_hparams_path(method: str, hparams_model: str, hparams_dir: str) -> Path:
    """Return the pinned EasyEdit hparameter file for a supported method."""
    method_upper = method.upper()
    if method_upper not in EASYEDIT_METHODS:
        raise ValueError(
            f"Unknown EasyEdit method: {method}. Available: {sorted(EASYEDIT_METHODS)}"
        )
    return Path(hparams_dir) / EASYEDIT_METHODS[method_upper][1] / f"{hparams_model}.yaml"
...
def resolve_pre_edit_run_config(args: argparse.Namespace) -> dict | None:
    """Resolve frozen-control output-preservation evaluation for custom runs."""
    size = int(getattr(args, "pre_edit_probe_size", 0))
    checkpoints = list(getattr(args, "pre_edit_checkpoints", None) or [])
    if args.method not in {"biocs", "ft"}:
        if size or checkpoints:
            raise ValueError(
                "pre-edit output-preservation controls are currently supported "
                "only by the matched custom biocs/ft evaluators"
            )
        return None
    if size < 0:
        raise ValueError("--pre_edit_probe_size must be non-negative")
    if checkpoints and size == 0:
        raise ValueError("--pre_edit_checkpoints requires --pre_edit_probe_size")
    if any(checkpoint <= 0 for checkpoint in checkpoints):
        raise ValueError("--pre_edit_checkpoints must contain positive edit counts")
    if any(checkpoint > args.n_edits for checkpoint in checkpoints):
        raise ValueError("--pre_edit_checkpoints cannot exceed --n_edits")
    if size == 0:
        return None
    return {
        "pre_edit_probe_size": size,
        "pre_edit_checkpoints": sorted(set(checkpoints)),
        "selection": "contiguous non-overlapping rows immediately after the edit stream",
    }
...
def resolve_biocs_run_config(
    ...
    resolve_recipe = _biocs_editor_module().resolve_recipe
...
def load_dataset(name: str, n_edits: int, offset: int = 0):
    ...
    normalize_knowedit_record = _biocs_editor_module().normalize_knowedit_record
...
def run_easyedit_method(
    ...
    if method_upper not in EASYEDIT_METHODS:
        raise ValueError(f"Unknown method: {method}. Available: {sorted(EASYEDIT_METHODS)}")

    cls_name, _ = EASYEDIT_METHODS[method_upper]
    yaml_path = easyedit_hparams_path(method_upper, hparams_model, hparams_dir)
    ...
    with isolated_easyedit_imports():
        ...
        hparams = HParamsCls.from_hparams(str(yaml_path))
...
def load_edit_stream_manifest(
    data_path: str,
    *,
    n_edits: int,
) -> dict | None:
    """Load an auditable randomized edit/control split when one is supplied."""
    path_text = os.environ.get("KE_STREAM_MANIFEST", "").strip()
    if not path_text:
        return None
    path = Path(path_text)
    if not path.is_file():
        raise FileNotFoundError(f"KE_STREAM_MANIFEST does not exist: {path}")
    manifest = json.loads(path.read_text(encoding="utf-8"))
    expected_hash = hashlib.sha256(Path(data_path).read_bytes()).hexdigest()
    if manifest.get("dataset_sha256") != expected_hash:
        raise ValueError("KE_STREAM_MANIFEST belongs to a different dataset file")
    edit_indices = manifest.get("edit_indices")
    probe_indices = manifest.get("pre_edit_indices")
    if not isinstance(edit_indices, list) or not isinstance(probe_indices, list):
        raise ValueError("KE_STREAM_MANIFEST requires edit_indices and pre_edit_indices lists")
    if len(edit_indices) != n_edits:
        raise ValueError(
            f"KE_STREAM_MANIFEST has {len(edit_indices)} edit rows but --n_edits={n_edits}"
        )
    if len(edit_indices) != len(set(edit_indices)) or len(probe_indices) != len(set(probe_indices)):
        raise ValueError("KE_STREAM_MANIFEST must not repeat an edit or control row")
    if set(edit_indices).intersection(probe_indices):
        raise ValueError("KE_STREAM_MANIFEST edit and pre-edit control rows must not overlap")
    return {**manifest, "path": str(path.resolve()), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()}


def load_pre_edit_probes(
    data_path: str,
    *,
    dataset_name: str,
    n_edits: int,
    data_offset: int,
    config: dict | None,
    stream_manifest: dict | None,
) -> list[dict]:
    """Select held-out control prompts without overlapping the edit stream."""
    if config is None:
        return []
    with open(data_path, encoding="utf-8") as handle:
        rows = json.load(handle)
    if stream_manifest is not None:
        indices = list(stream_manifest["pre_edit_indices"])
        if len(indices) != int(config["pre_edit_probe_size"]):
            raise ValueError(
                "KE_STREAM_MANIFEST pre-edit control count conflicts with --pre_edit_probe_size"
            )
    else:
        start = data_offset + n_edits
        stop = start + int(config["pre_edit_probe_size"])
        if stop > len(rows):
            raise ValueError(
                "The requested edit stream plus non-overlapping pre-edit probes exceeds "
                f"the dataset length ({stop} > {len(rows)})"
            )
    ...
    for row_index in indices:
        raw = rows[row_index]
        item = normalize_knowedit_record(raw)
        prompt = item["prompt"]
        probes.append(
            {
                "probe_id": f"{dataset_name}-row-{row_index}-" + hashlib.sha256(
                    prompt.encode("utf-8")
                ).hexdigest()[:16],
                "prompt": prompt,
            }
...
def capture_update_geometry_reference(editor, seed: int, max_rows: int = 256) -> list[dict]:
    """Freeze identical row subsets before editing for paired update geometry."""
    import torch

    references = []
    for name, module in editor._get_mlp_modules():
        weight = module.weight.detach()
        row_count = min(int(max_rows), int(weight.shape[0]))
        digest = hashlib.sha256(
            f"ssr-cosmos-update-geometry-v1|{name}|{seed}".encode("utf-8")
        ).digest()
        generator = torch.Generator(device="cpu")
        generator.manual_seed(int.from_bytes(digest[:8], "little") % (2**63 - 1))
        rows = torch.randperm(weight.shape[0], generator=generator)[:row_count]
        references.append(
            {
                "module": name,
                "rows": rows,
                "before": weight.index_select(0, rows.to(weight.device)).float().cpu(),
            }
        )
    if not references:
        raise RuntimeError("No editable MLP modules were available for update geometry")
    return references


def summarize_update_geometry(editor, references: list[dict]) -> dict:
    """Measure diversity and overlap of the cumulative edited weight rows."""
    import torch
    import torch.nn.functional as F

    modules = dict(editor._get_mlp_modules())
    deltas = []
    module_rows = []
    for reference in references:
        name = reference["module"]
        if name not in modules:
            raise RuntimeError(f"Editable module disappeared after editing: {name}")
        weight = modules[name].weight.detach()
        rows = reference["rows"]
        after = weight.index_select(0, rows.to(weight.device)).float().cpu()
        delta = after - reference["before"]
        deltas.append(delta)
        module_rows.append(
            {
                "module": name,
                "sampled_rows": int(delta.shape[0]),
                "mean_row_l2": float(delta.norm(dim=1).mean()),
            }
        )
    ...
    if matrix.shape[0] < 2:
        return {
            "protocol": "cumulative_edit_update_rows_v1",
            "n_modules": len(references),
            "n_sampled_rows": int(matrix.shape[0]),
            "update_effective_rank": 0.0,
            "update_mean_abs_offdiag_cosine": 0.0,
            "update_rms": 0.0,
    ...
    return {
        "protocol": "cumulative_edit_update_rows_v1",
        "definition": (
            "Effective rank uses squared singular values of the cumulative sampled-row "
            "update; overlap is mean absolute off-diagonal row cosine."
        ),
        "n_modules": len(references),
        "n_sampled_rows": int(matrix.shape[0]),
        "update_effective_rank": float(effective_rank),
        "update_mean_abs_offdiag_cosine": float(offdiag.abs().mean()),
        "update_rms": float(matrix.square().mean().sqrt()),
        "module_rows": module_rows,
    }


def attach_update_geometry(summary: dict, editor, references: list[dict], output_dir: str) -> dict:
    summary["update_geometry"] = summarize_update_geometry(editor, references)
...
def run_biocs_method(
    ...
    pre_edit_run_config: dict | None = None,
):
    """Run our SSR KE method."""
    sys.path.insert(0, str(PROJECT_DIR))
    editor_module = _biocs_editor_module()
    BioCsLLMEditor = editor_module.BioCsLLMEditor
    KnowEditDataset = editor_module.KnowEditDataset
    run_sequential_editing = editor_module.run_sequential_editing
    ...
    geometry_reference = capture_update_geometry_reference(editor, config["seed"])
    data_offset = int(os.environ.get("KE_DATA_OFFSET", "0"))
    stream_manifest = load_edit_stream_manifest(data_path, n_edits=n_edits)
    dataset = KnowEditDataset(
        data_path,
        max_samples=n_edits,
        offset=data_offset,
        indices=stream_manifest["edit_indices"] if stream_manifest else None,
    )
    probes = load_pre_edit_probes(
        data_path,
        dataset_name=dataset_name,
        n_edits=n_edits,
        data_offset=data_offset,
        config=pre_edit_run_config,
    ...
    summary = run_sequential_editing(
        ...
        **(history_run_config or {}),
        pre_edit_probes=probes,
        pre_edit_checkpoints=(pre_edit_run_config or {}).get("pre_edit_checkpoints"),
    )
    return attach_update_geometry(summary, editor, geometry_reference, output_dir)
...
def run_ft_method(
    ...
    sys.path.insert(0, str(PROJECT_DIR))
    editor_module = _biocs_editor_module()
    FTBaselineEditor = editor_module.FTBaselineEditor
    KnowEditDataset = editor_module.KnowEditDataset
    run_sequential_editing = editor_module.run_sequential_editing
    ...
    editor = FTBaselineEditor(
        ...
        device="cuda",
        target_layers=target_layers,
        # Keep the control's editable object identical to SSR while setting
        # every regularizer to zero in FTBaselineEditor.
        kernel_family=os.environ.get("BIOCS_KERNEL_FAMILY", "gaussian"),
        a_exc=float(os.environ.get("BIOCS_A_EXC", "1.0")),
        a_inh=float(os.environ.get("BIOCS_A_INH", "0.8")),
        sigma_exc=float(os.environ.get("BIOCS_SIGMA_EXC", "0.3")),
        sigma_inh=float(os.environ.get("BIOCS_SIGMA_INH", "0.8")),
        biocs_target=os.environ.get("BIOCS_TARGET", "weight"),
        distance_mapping=os.environ.get(
            "BIOCS_DISTANCE_MAPPING", os.environ.get("BIOCS_DISTANCE_METRIC", "cosine")
        ),
        row_selection=os.environ.get("BIOCS_ROW_SELECTION", "fixed_random"),
        max_spatial_rows=int(os.environ.get("BIOCS_MAX_SPATIAL_ROWS", "256")),
    ...
    geometry_seed = int(os.environ.get("KE_SEED", "0"))
    geometry_reference = capture_update_geometry_reference(editor, geometry_seed)
    data_offset = int(os.environ.get("KE_DATA_OFFSET", "0"))
    stream_manifest = load_edit_stream_manifest(data_path, n_edits=n_edits)
    dataset = KnowEditDataset(
        data_path,
...
def write_run_manifest(
    ...
    if pre_edit_run_config is not None:
        record["pre_edit_output_consistency_evaluation"] = pre_edit_run_config
    stream_path = os.environ.get("KE_STREAM_MANIFEST", "").strip()
    if stream_path:
        path = Path(stream_path)
        record["stream_manifest"] = {
            "path": str(path.resolve()),
            "sha256": hashlib.sha256(path.read_bytes()).hexdigest() if path.is_file() else None,
        }
    method = args.method.upper()
    if method in EASYEDIT_METHODS:
        hparams_path = easyedit_hparams_path(method, args.hparams_model, args.hparams_dir)
        record["hparams"] = {
...
def write_result_record(
    ...
    pre_edit_final = summary.get("pre_edit_output_consistency", {}).get("final")
    if isinstance(pre_edit_final, dict):
        metrics["pre_edit_output_consistency"] = float(pre_edit_final.get("score", 0.0)) * 100.0
    geometry = summary.get("update_geometry")
    if isinstance(geometry, dict):
        for key in (
            "update_effective_rank",
            "update_mean_abs_offdiag_cosine",
            "update_rms",
        ):
            if key in geometry:
                metrics[key] = float(geometry[key])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, required=True,
                        choices=["ROME", "MEMIT", "AlphaEdit", "SPHERE", "WISE", "biocs", "ft"])
    ...
    parser.add_argument("--hparams_dir", type=str, default=HPARAMS_DIR,
                        help="Directory containing EasyEdit method hparameter subdirectories.")
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Number of non-overlapping held-out prompts whose deterministic pre-edit "
            "continuations are frozen for output-preservation evaluation (custom biocs/ft)."
        ),
    )
    parser.add_argument(
        "--pre_edit_checkpoints",
        type=int,
        nargs="*",
        default=None,
        metavar="N",
        help="Edit counts at which frozen pre-edit control outputs are re-evaluated.",
    )
    ...
    try:
        ...
        parser.error(str(exc))
    try:
        pre_edit_run_config = resolve_pre_edit_run_config(args)
    except ValueError as exc:
        parser.error(str(exc))
    ...
    summary = None
    if args.method.upper() in EASYEDIT_METHODS:
