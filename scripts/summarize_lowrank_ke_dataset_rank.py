...
def summarize_values(baseline: list[float], ssr: list[float], key: str) -> dict:
    if len(baseline) != len(ssr) or not baseline:
        raise ValueError(f"Need one or more paired observations for {key}")
...
def performance_value(row: dict, metric: str) -> float | None:
    ...
    value = row["immediate"].get(immediate_key)
    return None if value is None else 100.0 * float(value)


def paired_performance_values(
    baseline_records: dict[int, dict],
    ssr_records: dict[int, dict],
    seeds: list[int],
    edit_count: int,
    metric: str,
    ...
    for seed in seeds:
        left = performance_value(checkpoint(baseline_records[seed], edit_count), metric)
        right = performance_value(checkpoint(ssr_records[seed], edit_count), metric)
        if left is None or right is None:
            continue
        baseline_values.append(left)
        ssr_values.append(right)
    return baseline_values, ssr_values
...
def main() -> None:
    ...
    for edit_count in CHECKPOINTS:
        ...
        for metric in PERFORMANCE_METRICS:
            baseline, ssr = paired_performance_values(
                records[0.0], records[args.ssr_lambda], seeds, edit_count, metric
            )
            performance[metric] = (
                summarize_values(baseline, ssr, f"{args.dataset_label}:{edit_count}:{metric}")
                if baseline
                else {
                    "baseline_mean": None,
                    "baseline_ci95": None,
        ...
        for metric in GEOMETRY_METRICS:
            ...
            geometry[metric] = summarize_values(
                baseline, ssr, f"{args.dataset_label}:{edit_count}:geometry:{metric}"
            )
                ...
                csv_rows.append(
                    {
                        ...
                        "delta_ci95_low": None if summary["delta_ci95"] is None else summary["delta_ci95"][0],
                        "delta_ci95_high": None if summary["delta_ci95"] is None else summary["delta_ci95"][1],
    ...
    history_auc_summary = summarize_values(
        baseline_auc, ssr_auc, f"{args.dataset_label}:history_auc"
    )
    ...
    payload = {
        "schema_version": "lowrank_lora_ssr_dataset_rank_transfer_v1",
        "protocol": "frozen_zsre_selected_rank_configuration_transfer",
        ...
        "reporting": (
            ...
            "from ZsRE before these ten new dataset-transfer streams. All paired outcomes are retained. "
            "Metrics absent from a dataset are reported as missing rather than coerced. Target-consistency locality, "
